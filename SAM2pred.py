...
class  SAM_pred(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.weightpath = args.weightpath
        self.benchmark = args.benchmark
        sam_vith_weightpath = os.path.join(self.weightpath, 'SAM/sam_vit_h_4b8939.pth')
        self.sam_model = sam_model_registry['vit_h'](sam_vith_weightpath)
        self.sam_model.eval()
    ...
    def get_feat_from_np(self, query_img, query_name, protos):
        np_feat_path = os.path.join(self.weightpath, 'SAM/')
        np_feat_path = os.path.join(np_feat_path, self.benchmark + '/')
