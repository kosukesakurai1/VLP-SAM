...
class DatasetPASCAL(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize, vlmtransform, model, args):
        ...
        if model.module.backbone_type in ['ViT-L/14', 'RN50', 'ViT-B/16', 'CS-ViT-B/16', 'CS-ViT-L/14', 'CS-RN50']:
            ...
            save_name = f'pascal_clip_text_{model.module.backbone_type}.npy'.replace('/', '-')
            if save_name not in files_name:
                label = ["aeroplane", "bicycle", "bird", "boat", "bottle", "bus", "car", "cat", "chair", "cow", "diningtable", "dog", "horse", "motorbike", "person", "potted plant", "sheep", "sofa", "train", "tv/monitor"]
            ...
            self.text_features = torch.from_numpy(np.load(text_feat_path + save_name)) # [20, 768] ('ViT-L/14')
        else:
    ...
    def __getitem__(self, idx):
        ...
        if not self.use_original_imgsize:
            query_cmask = F.interpolate(query_cmask.unsqueeze(0).unsqueeze(0).float(), sam_query_img.size()[-2:], mode='nearest').squeeze()
