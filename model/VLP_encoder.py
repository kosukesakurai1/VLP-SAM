r""" Vision and Language reference Prompt encoder of VLP-SAM """
...
class VLP_encoder(nn.Module):
    def __init__(self, args, backbone, use_original_imgsize, device):
        super(VLP_encoder, self).__init__()
        ...
        self.texttype = args.text
        if self.texttype == 'yes':
            feat_multi = 3
        elif self.texttype == 'no':
            feat_multi = 2
        else:
            raise Exception('Unavailable text word: %s' % self.texttype)
        ...
        if backbone == 'vgg16':
            ...
            deep_stem = False
            resnet = models.resnet50(pretrained=True, deep_stem=deep_stem)
            if deep_stem:
            ...
            self.clipmodel, self.clippreprocess = clip.load(backbone, device=device)
            fea_dim = 768
            ...
            self.clipmodel, self.clippreprocess = clip.load(backbone, device=device)
            fea_dim = 1024
        elif backbone == 'CS-ViT-B/16':
            self.clipmodel, self.clippreprocess = clip.load(backbone, device=device)
            fea_dim = 512
        elif backbone == 'CS-ViT-L/14':
            self.clipmodel, self.clippreprocess = clip.load(backbone, device=device)
            fea_dim = 768
        elif backbone == 'CS-RN50':
            self.clipmodel, self.clippreprocess = clip.load(backbone, device=device)
            fea_dim = 1024
        else:
        ...
        if backbone in ['CS-ViT-B/16', 'CS-ViT-L/14', 'CS-RN50'] and self.texttype == 'yes':
            self.merge_1 = nn.Sequential(
                nn.Conv2d(hidden_dim*feat_multi+2, hidden_dim, kernel_size=1, padding=0, bias=False),
                nn.ReLU(inplace=True),
            )
        else:
            self.merge_1 = nn.Sequential(
                nn.Conv2d(hidden_dim*feat_multi+1, hidden_dim, kernel_size=1, padding=0, bias=False),
                nn.ReLU(inplace=True),
            )
    ...
    def forward(self, condition, query_img, support_img, support_mask, training, vlm_query_img, text_feature):
        ...
        if self.backbone_type in ['ViT-L/14', 'RN50', 'ViT-B/16', 'CS-ViT-B/16', 'CS-ViT-L/14', 'CS-RN50']:
            with torch.no_grad():
                query_feat = self.clipmodel.encode_image(vlm_query_img).float()                 
                # support_img = support_img * support_mask.unsqueeze(1)
                supp_feat = self.clipmodel.encode_image(support_img).float()

                # create image-text attention map
                if self.texttype == 'yes':
                    text_feature = (text_feature / text_feature.norm(dim=-1, keepdim=True)).float()
                    if self.backbone_type in ['CS-ViT-B/16', 'CS-ViT-L/14', 'CS-RN50']:
                        B, C, H, W = query_feat.shape
                        query_features = query_feat.reshape(B, C, -1).permute(0, 2, 1) @ text_feature.t()
                        supp_features = supp_feat.reshape(B, C, -1).permute(0, 2, 1) @ text_feature.t()
                        query_similarity_map = clip.get_similarity_map(query_features, (H,W))
                        supp_similarity_map = clip.get_similarity_map(supp_features, (H,W))
                        query_attn_map = []
                        supp_attn_map = []
                        for i in range(B):
                        ...
                        supp_attn_map = torch.stack(supp_attn_map).unsqueeze(1)
                
                # create pseudo mask
                resize_size = supp_feat.size(2) * 3
                support_mask = F.interpolate(support_mask_ori.unsqueeze(1).float(), size=(resize_size, resize_size), mode='nearest')
            ...
            support_mask = F.interpolate(support_mask, size=(supp_feat.size(2), supp_feat.size(3)), mode='bilinear')
            ...
            if self.texttype == 'yes':
                ...
                text_feat = self.downsample_query(text_feature.unsqueeze(2).unsqueeze(3)).repeat(1, 1, query_feat.shape[2], query_feat.shape[3])
                if self.backbone_type in ['CS-ViT-B/16', 'CS-ViT-L/14', 'CS-RN50']:
                    supp_feat_1 = self.merge_1(torch.cat([supp_feat, supp_feat_bin, text_feat, support_mask*10, supp_attn_map*10], 1))                                                                                    
                    query_feat_1 = self.merge_1(torch.cat([query_feat, supp_feat_bin, text_feat, pseudo_mask*10, query_attn_map*10], 1))
                else:
                    supp_feat_1 = self.merge_1(torch.cat([supp_feat, supp_feat_bin, text_feat, support_mask*10], 1))                                                                                    
                    query_feat_1 = self.merge_1(torch.cat([query_feat, supp_feat_bin, text_feat, pseudo_mask*10], 1))
            elif self.texttype == 'no':
